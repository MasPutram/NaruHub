...
export default function TermuxPackagesPage() {
  ...
  const command = accessKey.trim()
    ? `bash <(curl -fsSL "https://naruhub.my.id/api/termux/bootstrap?key=${encodeURIComponent(accessKey.trim())}")`
    : "";
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .input-group label { display: block; color: var(--dim); font-size: 12px; font-weight: 700; margin-bottom: 6px; }
        .input-group input {
          width: 100%; max-width: 400px; background: var(--bg); color: var(--ink);
          border: 1px solid var(--card-border); border-radius: 8px; padding: 10px 12px; font-size: 14px;
        }
      ...
      <div className="subtitle">Generate command untuk menjalankan NaruHub di Termux / Cloud instance.</div>

      <div className="section">
        <div className="section-title">GENERATE COMMAND</div>
        ...
        {command ? (
          ...
          <div className="empty">Masukkan access key untuk generate command.</div>
        )}
      </div>
      ...
      <div className="section">
        ...
        <div className="steps">
          ...
          2. Paste command yang sudah di-copy di atas<br />
